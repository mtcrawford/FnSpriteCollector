namespace FNSpriteCollector.Shared;

public static class SpriteId
{
    // Base
    public const int Water = 1;
    public const int Earth = 2;
    public const int Fire = 3;
    public const int Duck = 4;
    public const int Ghost = 5;
    public const int Dream = 6;
    public const int Punk = 7;
    public const int King = 8;
    public const int Zero_Point = 9;
    public const int Demon = 10;
    public const int Burnt_Peanut = 11;
    public const int Fishy = 42;
    public const int Striker = 46;
    public const int Aura = 50;
    public const int Boss = 54;
    public const int Grim = 58;
    public const int Air = 67;
    public const int Seven = 72;
    public const int Batman = 77;
    public const int ViniJr = 82;
    public const int Pollo = 83;

    // Gold
    public const int Gold_Water = 12;
    public const int Gold_Earth = 13;  
    public const int Gold_Fire = 14;
    public const int Gold_Duck = 15;
    public const int Gold_Ghost = 16;
    public const int Gold_Dream = 17;
    public const int Gold_Punk = 18;
    public const int Gold_King = 19;
    public const int Gold_Zero_Point = 20;
    public const int Gold_Demon = 21;
    public const int Gold_Fishy = 43;
    public const int Gold_Striker = 47;
    public const int Gold_Aura = 51;
    public const int Gold_Boss = 55;
    public const int Gold_Grim = 59;
    public const int Gold_Air = 68;
    public const int Gold_Seven = 73;
    public const int Gold_Batman = 78;

    // Gummy
    public const int Gummy_Water = 22;
    public const int Gummy_Earth = 23;
    public const int Gummy_Fire = 24;
    public const int Gummy_Duck = 25;
    public const int Gummy_Ghost = 26;
    public const int Gummy_Dream = 27;
    public const int Gummy_Punk = 28;
    public const int Gummy_King = 29;
    public const int Gummy_Zero_Point = 30;
    public const int Gummy_Demon = 31;
    public const int Gummy_Fishy = 44;
    public const int Gummy_Striker = 48;
    public const int Gummy_Aura = 52;
    public const int Gummy_Boss = 56;
    public const int Gummy_Grim = 60;
    public const int Gummy_Air = 69;
    public const int Gummy_Seven = 74;
    public const int Gummy_Batman = 79;


    // Galaxy
    public const int Galaxy_Water = 32;
    public const int Galaxy_Earth = 33;
    public const int Galaxy_Fire = 34;
    public const int Galaxy_Duck = 35;
    public const int Galaxy_Ghost = 36;
    public const int Galaxy_Dream = 37;
    public const int Galaxy_Punk = 38;
    public const int Galaxy_King = 39;
    public const int Galaxy_Zero_Point = 40;
    public const int Galaxy_Demon = 41;
    public const int Galaxy_Fishy = 45;
    public const int Galaxy_Striker = 49;
    public const int Galaxy_Aura = 53;
    public const int Galaxy_Boss = 57;
    public const int Galaxy_Grim = 61;
    public const int Galaxy_Air = 70;
    public const int Galaxy_Seven = 75;
    public const int Galaxy_Batman = 80;

    // Holofoil
    public const int Holofoil_Water = 62;
    public const int Holofoil_Fire = 63;
    public const int Holofoil_Ghost = 64;
    public const int Holofoil_King = 65;
    public const int Holofoil_Striker = 66;
    public const int Holofoil_Air = 71;
    public const int Holofoil_Seven = 76;
    public const int Holofoil_Batman = 81;
}